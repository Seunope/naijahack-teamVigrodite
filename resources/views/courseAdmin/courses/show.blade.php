...
@section('title', 'Admin course details page')
...
<div id="page-wrapper">
    ...
    <div class="row">
        ...
        <div class="col-md-6">
                <h5><strong>Course</strong>: <i>{{$course->code or ''}} ({{$course->title or ''}})</i></h5>
                ...
                <h5>Cordinator :<i>{{$course->user->name or ''}}</i> </h5>

                <h5>Type :<i>@if($course->type==0)ppt @else cbt @endif</i> </h5>
                
                <h5>Unit :<i>{{$course->unit or ''}}</i> </h5>
                        
                <a href="/courseAdmin/courses/{{$course->slug}}/edit">Edit</a>
