...
<div id="page-wrapper">
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <h5><strong>Faculty</strong>: <i>{{$department->faculty->name or ''}} -{{$department->faculty->school->name or ''}}</i></h5>
            
            <h5><strong>No of Courses in this department</strong>: <i>{{count($department->courses)}}</i></h5>
            ...
            <!-- <a href="{{URL::to('/departmentAdmin/departments')}}" class="btn btn-success btn-sm back-button"><i class="fa fa-arrow-left"></i> Back</a> -->
            @if(count($department->courses)>0)
            <div class="list-group">
                <h4>List of Courses under {{$department->name or ''}} Department.</h4>
                @foreach($department->courses as $course)
                <a href="/departmentAdmin/courses/{{$course->slug or ''}}" class="list-group-item">{{$num++}}). {{$course->code or ''}}-  {{$course->title or ''}}</a>
                @endforeach
            </div>
            @else
